...
## ✅ 완료 · 배포됨 (추가분 2 — 노인일자리)
- [x] **노인일자리 페이지 신설**(welfare 복사) — `jobs.php` + `includes/senuri.php`. 한국노인인력개발원 100세누리 구인정보 API(data.go.kr `B552474/SenuriService`). getJobList numOfRows=1000 단건(~9s)으로 **접수중 367건** 적재·30분 캐시, getJobInfo 상세. 히어로·검색·인기칩·지역필터·카드·페이지네이션·상세 모두 welfare 톤. 네비/좌측레일 '노인일자리' 추가. **검증: 367건·카드필드·지역칩·상세 정상.**
  - 인증키: 서버 `includes/senuri.key`(64B)에서만 읽음 — 리포 미포함(.gitignore). 30분 캐시 워밍 크론(웹 경로) 등록.

## ⏳ 내일 이어서 (노인일자리 후속)
- [ ] **jobs.php 상세를 모달로 전환** — 현재는 `jobs.php?id=` 풀페이지 상세. 카드 클릭 시 AJAX(`?id=X&ajax=1` 프래그먼트)로 **모달** 오픈(문의 모달 패턴 참고). `?id=` 풀페이지는 fallback/SEO로 유지. **← 여기서 중단함.**
- [ ] 지역칩 "전남광주" 이상값 정리(일부 workPlcNm이 "전남광주…" 무공백 → senuri_sido 보정).
