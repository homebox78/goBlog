...
async function autoGenerateDailyMix(): Promise<number> {
  const { getSettingValues } = await import("../settings/settings.service.js");
  const cfg = await getSettingValues(["scheduler.dailyLimit", "affiliate.bannersEnabled"]);
  const limit = Math.max(0, Number(cfg["scheduler.dailyLimit"] ?? 5) || 5);
  if (limit === 0) return 0;
  const perSection = Math.max(1, Math.floor(limit / NEWS_SECTIONS.length)); // 5→섹션당 1, 10→2
  ...
  let generated = 0;
  for (let round = 0; round < perSection && budget > 0; round += 1) {
    for (const s of NEWS_SECTIONS) {
      if (budget <= 0) break;
      if (made[s] >= perSection) continue;
  ...
  console.log(
    `[scheduler] 카테고리별 자동 생성: ${NEWS_SECTIONS.map((s) => `${s} ${made[s]}/${perSection}`).join(" · ")} (이번 회차 ${generated}건)`,
  );
  return generated;
}
...
export async function dripPublishDaily(): Promise<number> {
  ...
  const order: NewsSection[] = [...NEWS_SECTIONS];
