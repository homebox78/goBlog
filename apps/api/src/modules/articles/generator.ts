...
export interface GenerateOptions {
  ...
  /** 핵심 논지·관점 — 지정하면 이 관점을 글의 결론 방향으로 강제한다(사실은 왜곡 금지, 이 논지의 근거로 해석). */
  angle?: string;
...
const ARTICLE_TYPE_GUIDE: Record<string, string> = {
  ...
  "investment-analysis":
    "투자·재테크 분석 — '무엇이다'가 아니라 '왜 그럴 수밖에 없는가'를 인과로 밀어붙이는 날카로운 논증(사실·수치는 근거 기반, 종목 리딩 금지)",
...
export async function generateArticle(
  ...
  const claudeSystem = [
      ...
      "",
      "[AEO/GEO — AI 답변엔진(ChatGPT·Gemini·구글 AI개요)이 인용하기 좋게. 반드시 준수]",
      "- 독자가 실제로 검색·질문하는 문장을 H2 소제목으로 만든다(예: '~는 얼마인가요?', '~와 ~의 차이는?', '~하는 방법은?'). 최소 2~3개 H2를 질문형으로.",
      "- 질문형 H2 바로 다음 문단은 **결론부터 한두 문장으로 명확하게** 답한다(AI가 그 문단을 그대로 인용한다). 배경 설명은 그 뒤에 붙인다.",
      "- 수치·금액·기준·일정은 두루뭉술('많이', '대체로')하지 말고 **구체적 숫자와 단위**로 쓴다(예: '평균 100만~500만원', '2026년 기준 4.75%'). AI는 근거가 명확한 문장을 우선 인용한다.",
      "- 비교·조건·단계는 표나 번호목록으로 정리해 기계가 파싱하기 쉽게 만든다.",
      "",
      style
        ? [
            "",
      ...
      options.articleType === "investment-analysis"
        ? [
            "",
            "[💹 투자 분석 앵글 — 이 글은 '정보 정리'가 아니라 '하나의 논증'이다. 아래 규칙이 위의 감정·공감 오프너·응원 마무리 지시보다 우선한다]",
            "- **테제부터 던진다**: '무엇이다'를 나열하지 말고 '왜 그럴 수밖에 없는가'를 인과로 밀어붙인다. 첫 문단에 도발적 질문·역설·핵심 주장을 세운다 (예: '왜 삼성바이오로직스는 비만약 CDMO를 피할 수 없나').",
            "- **오프너 교체**: 위의 '막막함·불안을 짚는 감정 공감형' 오프너를 쓰지 않는다. 대신 시장의 긴장·모순·판돈(무엇이 걸려 있나)을 첫 문장에 세운다.",
            "- **인과로 전개**: ① 시장 구조·경쟁·유인(왜 이런 판이 됐나) → ② 그래서 이 기업·자산이 취할 수밖에 없는 선택 → ③ 반론·리스크(무엇이 이 논증을 깨나) → ④ 투자 시사점. 각 H2 소제목이 이 논증의 한 단계가 되게 한다.",
            "- **냉정하게**: 응원·미사여구·'힘내세요'류를 뺀다. 근거·수치·인과로만 설득한다. `## 마무리`도 따뜻한 인사 대신 **판단(투자 시사점: 무엇을 봐야 하나, 무엇이 신호이고 무엇이 함정인가)**으로 맺는다.",
            "- **읽을 가치**: 검색하면 나오는 뻔한 '○○란·전망은?' 요약과 정반대로, 남들이 놓친 인과·구조·이해관계를 짚어 '이래서 이렇게 될 수밖에 없구나'를 남긴다.",
            "- ⚠️ **논증은 날카롭게, 사실·수치·전망은 recentNews·encyclopedia 근거로만.** 도발적이라고 실적·수치·미래를 지어내지 않는다. 확인 안 된 예측은 '가능성'으로만 쓰고 단정하지 않는다.",
            "- 🚫 특정 종목 매수·매도 권유(리딩)·목표주가 단정·'세력'·'급등 예약'·'확정 수익' 같은 선동/과장은 절대 금지(정책 위반). 구조와 논리를 보여줄 뿐, 투자 판단은 독자 몫임을 전제한다.",
          ].join("\n")
        : "",
      "",
      "[🎯 독창성·경험 — 흔한 정보 글이 되지 않게 (매우 중요)]",
      "- 이 주제로 검색하면 나오는 뻔한 글(사전적 정의로 시작, 공식 안내문 요약, 같은 목차)과 **다른 글**이어야 한다. citationInsight의 alreadyCoveredAngles는 피하고 openGapsToAttack에서 각도를 잡는다.",
      "- **조사 과정을 경험 서사로 쓴다** — 이 글을 쓰며 실제로 한 일(여러 보도·공식 자료를 비교하고, 조건을 따져보고, 금액을 계산해 본 것)을 1인칭으로 드러낸다: '세 곳 보도를 비교해 보니 수치가 달랐습니다', '약관을 끝까지 읽어보면 이런 단서가 있습니다', '직접 계산해 보면 이렇게 됩니다'. 이것은 실제 수행한 조사라 허위가 아니다.",
      "- **직접 만든 계산 사례·시나리오**를 최소 1개 넣는다: 구체적 인물 설정('만 34세 직장인, 연봉 4천이라면')으로 실제 숫자를 대입해 결과를 보여준다. 이 계산이 글의 독창적 가치가 된다.",
      "- **자기만의 판단 기준**을 제시한다: 남들이 나열만 할 때 우리는 '이 조건이면 A, 저 조건이면 B'처럼 기준을 세워 정리하거나, 항목별로 별점·우선순위를 매긴다.",
      "- ⚠️ 단, 하지 않은 일은 여전히 지어내지 않는다: 제품을 써봤다·현장에 가봤다·신청해서 받았다 같은 **수행하지 않은 경험 단정은 금지**. 경험 서사는 '자료를 읽고 비교하고 계산한' 실제 조사 행위까지만.",
      "",
      "[사람이 쓴 것처럼 — AI 티 최소화]",
      ...
      "",
      "[마무리 인사이트 + 참고자료 — FAQ 뒤 고정 구성. ⚠️ 본문 마지막 순서: FAQ → ## 마무리 → ## 참고자료 → 해시태그 줄(#태그들, 반드시 유지·맨 끝)]",
      "- 본문 마지막에 `## 마무리` 섹션(2~4문장)을 쓴다. 요약 반복이 아니라, 이 주제를 관통하는 **따뜻한 인사이트 한 조각**으로 끝낸다 — 독자의 상황에 공감하고, 판단의 중심을 짚어주고, 응원으로 맺는다. (예: '트렌드와 통계는 출발점일 뿐, 결국 중요한 건 ~입니다. ~하시길 응원합니다.')",
      "- 마무리 다음에 `## 참고자료` 섹션을 쓴다. ⚠️ **userPayload의 recentNews(제목·link)와 encyclopedia(url)에 실제로 있는 출처만** 목록으로 넣는다 — `- [매체·글 제목](링크)` 형식. 본문 작성에 실제로 참고한 것 2~4개만 고른다.",
      "- **출처가 하나도 없으면 참고자료 섹션을 아예 만들지 않는다.** 링크·매체명·기관명을 지어내는 것은 절대 금지 — 가짜 출처는 글 전체를 허위로 만든다.",
      "- 대법원·국세청 같은 공식 기관 사이트는 recentNews에 없어도 **대표 도메인이 확실할 때만**(예: scourt.go.kr, nts.go.kr) 언급할 수 있다. 확신 없으면 이름만 쓰고 링크는 생략한다.",
      ...
      "",
      "[❤️ 감정·사람 냄새 — '정보 딕셔너리'가 아니라 '사람이 건네는 이야기'로]",
      "- 독자가 지금 어떤 마음으로 이 글을 검색했는지 먼저 헤아린다: 막막함·불안·짜증·조급함·설렘·궁금함. 그 감정을 첫머리에서 한 번 콕 짚어 '내 얘기네' 싶게 만든다.",
      "- 정보를 나열만 하지 말고, 곳곳에 **사람의 온도**를 싣는다: 솔직한 심경('저도 이거 처음엔 헷갈렸어요'는 금지 — 없는 경험은 안 지어냄. 대신 '이 부분, 은근 헷갈리죠'처럼 독자 감정에 얹는다), 작은 공감('맞아요, 이럴 때 진짜 답답하죠'), 담담한 응원('조급해하지 않아도 됩니다').",
      "- 딱딱한 접속사(따라서·그러므로·또한) 대신 말하듯 잇는다(그래서·근데·사실·솔직히·의외로). 혼잣말 같은 짧은 문장을 가끔 던진다('이게 은근 중요해요.').",
      "- 감정은 **담백하게**. 오글거리는 미사여구·과장('정말 놀라운'·'충격적인'·'대박')·억지 감탄은 금지. 진짜 사람은 담담하게 말할 때 더 진심이 느껴진다.",
      "- ⚠️ 단, **수치·정보·판단 기준을 담은 문장은 감정 없이 정확하게** 쓴다(그 문장이 그대로 인용된다). 감정은 도입·연결·문단 마무리에만 싣는다.",
      "",
      "[독자를 사로잡는 글쓰기]",
      ...
      "[🔗 공식 사이트·서비스 링크 — 절차 안내에서 특히 중요]",
      "- 본문에서 공식 사이트·서비스·기관을 언급하면 **마크다운 링크로 건다**: `[엠세이퍼](https://www.msafer.or.kr)`. 이름만 적고 넘어가지 않는다 (독자가 주소를 직접 찾아야 하면 글의 값이 떨어진다).",
      "- 단계별 절차를 쓸 때, 접속해야 하는 사이트가 있으면 그 단계 안에서 링크를 건다. 예: `1단계 — 포털에서 '엠세이퍼' 검색 또는 [msafer.or.kr](https://www.msafer.or.kr) 직접 접속합니다.`",
      "- ⚠️ **주소를 지어내지 않는다.** 확실히 아는 공식 도메인만 링크로 걸고, 확신이 없으면 링크 없이 기관·서비스 이름만 쓰고 '공식 사이트에서 확인'으로 안내한다. 틀린 링크는 링크가 없는 것보다 나쁘다.",
      "- 링크는 공식 도메인만 건다 (블로그·카페·뉴스 기사·제휴 페이지로 연결하지 않는다).",
    ...
    JSON.stringify({
      ...
      citationInsight: insight
        ? {
            ...
            // 인용 상위 글들이 경험·독창성을 표현하는 방식 실측 — 우리 글의 경험 서사에 참고
            howTheyShowExperience: insight.personalVoice ?? null,
            whatMakesThemOriginal: insight.originality ?? null,
          }
        : null,
      ...
      outputFormat: {
        ...
        excerpt: "핵심 요약 — 독자가 3초 만에 파악할 핵심 포인트 3개. 각 포인트를 줄바꿈(\\n)으로 구분한 완결된 한 문장으로 쓰고, 사실·수치를 담는다. 목록 기호(·, -, 숫자)는 붙이지 않는다. (예: '첫째 핵심 문장\\n둘째 핵심 문장\\n셋째 핵심 문장')",
  ...
  if (quality.score <= 82) {
    // 어떤 항목이 감점됐는지 함께 보여준다 — "81점 폐기"만으로는 원인을 알 수 없다
    const failed = quality.items
      .filter((item) => !item.ok)
      .map((item) => item.label + (item.note ? `(${item.note})` : ""))
      .join(" · ");
    throw new HttpError(
      422,
      `품질 ${quality.score}점(82점 이하)으로 폐기되었습니다. 감점: ${failed || "없음"} — 다시 생성하거나 키워드를 바꿔주세요.`,
    );
  }
