...
//
// 웹앱 → window.postMessage({ type:"GOBLOG_NAVER_FETCH", id, url })  (url:"__ping__"은 감지용)
// 다리 → background fetch → window.postMessage({ type:"GOBLOG_NAVER_FETCH_RESULT", id, ok, html|error })

// 재주입(onInstalled로 기존 탭에 다시 주입) 대비 중복 리스너 방지
if (!window.__goblogBridge) {
  window.__goblogBridge = true;
  ...
  window.addEventListener("message", (event) => {
    ...
    // 감지용 핑 — background까지 안 가고 즉답
    if (data.url === "__ping__") {
      window.postMessage(
        { type: "GOBLOG_NAVER_FETCH_RESULT", id: data.id, ok: true, ping: true, version: chrome.runtime.getManifest().version },
        "*",
      );
      return;
    }
...
// 웹앱이 '확장 있음'을 감지할 수 있게 마커를 심는다 (재주입 시 버전 갱신)
