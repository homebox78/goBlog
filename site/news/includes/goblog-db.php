...
{
    $c = $category ?? '';
    // 큐레이션 키워드 category는 섹션명과 1:1 — 그대로 반환
    if (in_array($c, NEWS_SECTIONS, true)) return $c;
    ...
    if (preg_match('/해외.*축구|손흥민|이강인|프리미어|EPL|챔피언스|분데스|라리가/iu', $c)) return '해외축구';
    if (preg_match('/축구|K리그|국가대표|월드컵/iu', $c)) return '축구';
    if (preg_match('/농구|배구|KBL|V리그/iu', $c)) return '농구·배구';
    if (preg_match('/골프|UFC|올림픽|테니스|e스포츠|스포츠/iu', $c)) return '스포츠일반';
    return '연예가화제'; // 기본
}
